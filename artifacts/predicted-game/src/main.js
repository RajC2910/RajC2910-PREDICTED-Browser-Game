...
class PredictedGame {
  ...
  bind() {
    ...
    $("#multi-next-button").addEventListener("click", () => { this.audio.click(); if (this.levelIndex < LEVELS.length - 1) this.startMulti(this.levelIndex + 1, false); else this.toMenu(); });
  ...
  completeMulti() {
    ...
    $("#multi-next-button").textContent = this.levelIndex < LEVELS.length - 1 ? `ENTER ROOM ${pad(this.level.level + 1)}` : "RETURN TO MENU";
    $("#multi-again-button").textContent = "REPLAY ROOM";
